...
def move():
    ...
    if len(total_colors_detected) != 0:
...
def detect_letter(image):
    try:
        ...
        if letter in ("H", "I", "4"):
            ...
            # check rotations if letter is not a victim
            """
            degrees = 0
            while degrees <= 360:
                # rotate image
        ...
        return "no letter victim in sight"

def scan():
    toggle()
    if time.time()-run_start > run_length:
        return_to_start()
...
def go():
    ...
    if time.time()-run_start > run_length:
        return_to_start()
    atWalls = get_atWalls()
    pathWalls = get_pathWalls()
    exitDir = find_exitDir(pathWalls)
    print(f"Position: {pos}")
    print(f"scanned: {scanned_tiles}")
    print(f"atWalls: {atWalls}")
    print(f"pathWalls: {pathWalls}")
    print(f"neighbors: {get_neighbors()}")
    print(f"exitDir: {exitDir}")
    print(f"holes: {holes}")
    print(f"checkpoint: {checkpoint}, index {checkpoint_index}")
    if pos in scanned_tiles:
        if atWalls["front"]:
            toggle()
        if exitDir==270:
            turn_left(90)
