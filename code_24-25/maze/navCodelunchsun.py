...
##############
    ...
    while time.time()-start_time<moveTime:
        ...
        if ((br_rng[0] <= colorfound[0] <= br_rng[1]) and (colorfound[1] > 12)) or (bl_rng[0] <= colorfound[0] <= bl_rng[1]): # red or black
...
#        print(blueswitch, cpswitch, 5678)
        ...
        if special_rng[0] <= clear <= special_rng[1]:
            ...
            if red > 12:
                print("saw red in special range, THIS SHOULD NEVER HAPPEN, IF YOU SEE THIS SOMETHING WENT HORRIBLY WRONG")
            else:
