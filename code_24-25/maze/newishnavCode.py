...
def get_color(m=0):
    if m==0:
        ...
        if special_rng[0] <= clear <= special_rng[1]:
            if lux<2200:
                print()
############## COME BAXK HERE TO DO LL THE EDIST
        ...
        if special_rng[0] <= clear <= special_rng[1]:
            if lux<2200:
                print("saw silver in special rng")
                return "s"
            else:
                print("saw white in special rng")
                return "w"
        if bl_rng[0] <= clear <= bl_rng[1]:
            print("saw black")
            return "bl"
...
run_start = time.time()
going = True
while time.time()-run_start < run_length:
    go()
